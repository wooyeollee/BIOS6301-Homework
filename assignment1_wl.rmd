...
We can create a model using our data set.  In this case you would like to
...
```
By looking at the output, gender has the highest effect on 'los.' The scores of the male group are higher than those of the female group by 4.51 on average.
...
```{r}
coef(summary(mod))
tvalue <- coef(summary(mod))[2,3]
tvalue
(pt(tvalue, 7, lower.tail=FALSE))*2
```
...
t.test(women$los, men$los, var.equal=TRUE) # This is the model from summary
```
